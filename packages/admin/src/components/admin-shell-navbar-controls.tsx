 ...
 *
 * Pure-presentational Vue functional component: reads the preference store
 * and the nearest component Composer directly and calls the store action on
 ...
 */
export function AdminShellNavLeft(): VNode {
 ...
 *
 * Pure-presentational Vue functional component: reads the preference and
 * auth stores plus the nearest component Composer directly and invokes store
 * actions on selection. Rendered as a descendant of AdminShell so those
 * composition calls resolve against its context; no props or callback data.
 *
 * @returns The `nav-right` slot's JSX.
 */
export function AdminShellNavRight(): VNode {
  const preferences = useAdminShellPreferencesStore();
