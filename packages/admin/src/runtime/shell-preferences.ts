...
export type AdminShellPreferencesStorage = Pick<Storage, 'getItem' | 'setItem' | 'removeItem'>
...
const themeModeValues = ['light', 'dark', 'system'] as const satisfies readonly AdminThemeMode[]
const fontSizeValues = ['small', 'medium', 'large'] as const satisfies readonly AdminFontSize[]
...
const localeOptionsSchema = z.array(z.unknown()).catch([]).transform((items) =>
  items.flatMap((item) => {
    const parsed = localeOptionSchema.safeParse(item)
    return parsed.success ? [parsed.data] : []
  })
)
const themeModeInputSchema = themeModeSchema.catch(DEFAULT_THEME_MODE).default(DEFAULT_THEME_MODE)
const fontSizeInputSchema = fontSizeSchema.catch(DEFAULT_FONT_SIZE).default(DEFAULT_FONT_SIZE)
const localeInputSchema = z.union([nonEmptyStringSchema, z.undefined()]).catch(undefined)
...
type PersistedShellPreferences = z.infer<typeof persistedShellPreferencesSchema>
...
export function loadAdminShellPreferences(
  ...
  try {
    const persisted = parsePersistedShellPreferences(JSON.parse(serialized) as unknown)
...
function parsePersistedShellPreferences(input: unknown): PersistedShellPreferences | null {
  const parsed = persistedShellPreferencesSchema.safeParse(input)
  return parsed.success ? parsed.data : null
}

function cloneShellPreferences(preferences: AdminShellPreferences): AdminShellPreferences {
  return {
    ...preferences,
