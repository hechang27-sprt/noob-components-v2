...
import type { AdminAuthActions, AdminAuthStatus } from '../src/runtime-contract'
...
function mountLoginPage(authStatus: AdminAuthStatus, authActions: AdminAuthActions): HTMLElement {
...
describe('AdminLoginPage', () => {
  it('submits the entered login values through the injected auth action', async () => {
    ...
    const [usernameLabel, passwordLabel] = container.querySelectorAll<HTMLLabelElement>('label')
  ...
  it('gives each instance distinct label associations', () => {
    ...
    const usernames = [...container.querySelectorAll<HTMLInputElement>('input[name="username"]')]
    const passwords = [...container.querySelectorAll<HTMLInputElement>('input[name="password"]')]
  ...
  it('renders non-login states from the frontend auth status', () => {
    ...
    expect(loading.querySelector('h1')?.textContent).toBe('Checking your session')
    expect(loading.querySelector('[role="status"]')?.getAttribute('aria-live')).toBe('polite')
    expect(loading.querySelector('form')).toBeNull()
    expect(authenticated.textContent).toContain('Signed in as Ada Lovelace.')
    expect(authenticated.querySelector('h1')?.textContent).toBe('Already signed in')
  ...
  it('shows a generic error when the injected auth action rejects', async () => {
    ...
    expect(login).toHaveBeenCalledWith({ username: 'ada', password: 'bad-password', remember: false })
