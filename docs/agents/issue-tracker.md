...
- **List external PRs for triage**: `gh pr list --state open --json number,title,body,labels,author,authorAssociation,comments`; retain only external contributor associations.
...
- **Child ticket**: a GitHub sub-issue linked to the map. If sub-issues are unavailable, use a task-list entry and add `Part of #<map>` to the child. Label it `wayfinder:<type>`.
