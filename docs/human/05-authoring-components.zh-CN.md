...
组件从 CSS 自定义属性读取值。registry 包把声明的默认值映射为变量名。**并不存在全局主题 store**。与 naive-ui 的 `useThemeVars()` 不同，没有任何 API 可以让你取回默认的主题变量值。默认值位于组件源码中，并在每次调用时传给辅助函数。
...
在 JSX 中使用它们。这三个辅助函数是防漂移保护：参数类型被限制为结构体推导出的确切 CSS 变量名（`backgroundColor` → `--noob-ui-…-background-color`）。如果你重命名了结构体属性，使用旧变量名的引用会无法通过类型检查，因此过期的 JSX 字符串会在编译时被标记，而不是悄悄失效。你需要更新被标记的用法；不会有任何自动更新。
