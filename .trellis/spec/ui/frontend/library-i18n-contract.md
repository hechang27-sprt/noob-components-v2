...
- Hosts providing an override snapshot must defensively copy caller options (the admin AdminProvider does via structuredClone and the ConfigProviders via es-toolkit merge). Later caller mutation must not affect mounted or subsequently mounted components.
