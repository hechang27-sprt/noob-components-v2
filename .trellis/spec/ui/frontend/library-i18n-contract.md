...
- A no-build workspace application that aliases the library to source must precompile the source locale JSON in its own Vite build. Import package-owned resource include globs from a stable package Vite subpath; never hard-code `../../packages/<library>/src/locales/**` in the host.
...
| Source-consumer Vite include omits package JSON | Build/precompilation verification fails; import the package-owned Vite resource include. |
| Source-consuming host hard-codes a package-relative locale path | Replace it with the package-owned Vite integration export. |
...
- **Good:** component-first JSON; empty local Composer; defaults then component override slice; local `fallbackRoot` correction; immutable override snapshot; host owns active/fallback locale; standalone library build owns its precompilation; source-consuming workspace uses a package-owned Vite resource export.
...
- Source-consuming application typecheck and build with the package source alias and package-owned Vite resource include active.
- Resolve the package Vite subpath before building the package to prove no-build workspace compatibility.
...
// Correct for no-build source consumption: package owns the path contract.
import { adminI18nResourceInclude } from "@noob-naive-ui/admin/vite"
```
