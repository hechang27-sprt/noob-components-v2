...
- `@noob-naive-ui/admin` imports no router API and owns no persistence parser. The application owns route definitions, codecs, tab presentation, authentication, and history-scope policy. When using `@noob-naive-ui/admin-vue-router`, its `createAdminShellVueRouterNavigation` factory stamps the host's transient `getNavigationScopeId()` beside adapter tab metadata (`id`, `label`, and optional `closable`) beneath `_noobAdminShell`; metadata from another scope cannot restore tab identity. Codecs validate and map payload independently. The adapter reads `router.options.history.state`, not `window.history.state`, so browser and memory histories share one boundary.
