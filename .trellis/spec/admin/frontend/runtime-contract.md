...
- `@noob-naive-ui/admin` imports no router API and owns no persistence parser. The application owns route definitions, codecs, and tab presentation. When using `@noob-naive-ui/admin-vue-router`, its `createAdminShellVueRouterNavigation` factory owns generic router/history orchestration: codecs validate and map `payload` to Vue Router `params`, `query`, `hash`, `state`, or a mix; adapter metadata is limited to `id`, `label`, and optional `closable` beneath its reserved `_noobAdminShell` history-state namespace. It never persists a complete descriptor, `navKey`, or payload. The adapter reads `router.options.history.state`, not `window.history.state`, so browser and memory histories share one boundary.
