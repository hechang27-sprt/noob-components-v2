...
- `useAdminShell()` resolves the nearest mounted shell and returns one stable `AdminShellContext`: `navigate` uses that shell's existing destination-request path, while readonly computed `active` exposes only the host-authoritative public descriptor. It throws outside an `AdminShell`; the provider key stays private, and concurrent or nested shells remain isolated by Vue's hierarchical injection.
