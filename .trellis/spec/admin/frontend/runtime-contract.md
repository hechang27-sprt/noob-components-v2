...
- `useAdminShell()` resolves the nearest mounted shell and returns the full tabs controller (`AdminShellContext`): `navigate` (the destination-request path), plus shell-owned tab state and actions — `tabs`, `visibleTabs`, `tabError`, `canActivateTab`, `activateTab`, and `closeTab`. Descendants (the tabbar, page components) consume the controller directly instead of receiving callback props. Host-authoritative `navigation.active` remains internal shell input and is not re-exposed to descendants. The composable throws outside an `AdminShell`; the provider key stays private, and concurrent or nested shells remain isolated by Vue's hierarchical injection.
