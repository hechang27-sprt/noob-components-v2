...
- **Good:** `routes.tsx` exposes one dedicated object whose keys are both stable shell `navKey` values and equivalent Vue Router route names; URL paths remain separate. Every route definition owns a bidirectional URL codec: `encode` maps `AdminShellDestination.params` into explicit path/query/hash state for `handleNavigation()`, while `decode` reconstructs canonical destination params from the normalized URL for `currentDescriptor()`. Parameterless codecs reject hidden params. Shared registry helpers perform both directions, route records derive `name` from each map key, and `App.tsx` contains no route-specific parameter branching or reverse index. It creates plain menu labels, passes one stable navigation adapter, renders `RouterView` directly beneath `AdminShell`, and supplies its existing `logout` callback. Routed pages call command-only `useAdminShell().navigate(...)`; destination-defining values are consumed as route props rather than read from shell active state. A page-owned button may open a registered destination omitted from `menuOptions`; pass a second-argument resolver returning `{ kind: "open" }` when each click must create a distinct page instance. Destinations remain data-only.
