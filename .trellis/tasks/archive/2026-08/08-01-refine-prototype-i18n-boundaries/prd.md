...
3. Replace the demo's direct `../../packages/.../src/locales/**` include with a package-owned Vite integration export. Source-consuming workspace hosts use that stable export; consumers of a built package require no library-source include.
...
- [x] `apps/demo/vite.config.ts` contains no package-relative `src/locales` path and receives the source locale include through a package-owned Vite subpath export.
